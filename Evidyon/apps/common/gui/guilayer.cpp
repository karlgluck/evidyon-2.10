...
void GUILayer::destroy()
{
  ...
  if (myD3DDevice) myD3DDevice->Release();
...
void GUILayer::setFullscreenViewport(float minZ, float maxZ)
{
  ...
  if (!myD3DDevice) return;

  // Get the device's size
  LPDIRECT3DSURFACE9 backBufferSurface;
  if (FAILED(myD3DDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &backBufferSurface)))
    return;
...
{
  if (myD3DDevice == NULL) return true; // not initialized yet
    ...
    for (TextureToGUIElementTable::iterator i = myElements.begin(); i != myElements.end(); ++i)
    {
      ...
      if (i->first != currentTextureIndex)
      {
        ...
        if (currentTexture != NULL)
          currentTexture->cleanStage0Texture(myD3DDevice);
        else
        ...
        if (currentTexture != NULL)
          currentTexture->initStage0Texture(myD3DDevice);
      }
...
bool GUILayer::flush()
{
  ...
  HRESULT hr = myD3DDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, myConsumedVertices,
...
bool GUILayer::initializeD3DDevice()
{
  ...
  myD3DDevice->SetSamplerState(0, D3DSAMP_ADDRESSW, D3DTADDRESS_WRAP);
  ...
  myD3DDevice->SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_NONE);
  ...
  myD3DDevice->SetTextureStageState(0, D3DTSS_ALPHAOP,   D3DTOP_MODULATE);
  myD3DDevice->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
  myD3DDevice->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE);
  ...
  // on the edges.
  myD3DDevice->SetRenderState(D3DRS_ALPHATESTENABLE,  TRUE);
  myD3DDevice->SetRenderState(D3DRS_ALPHAREF,     0x08);
  ...
  myD3DDevice->SetFVF(GUIVERTEX_D3DBUFFERFORMAT);
  myD3DDevice->SetStreamSource(0, myVB, 0, sizeof(GUIVertex));
  myD3DDevice->SetIndices(myIB);

  // Set up the viewport
  myD3DDevice->SetViewport(&myViewport);

  // Set the camera
  myD3DDevice->SetTransform(D3DTS_PROJECTION, &myProjectionMatrix);

  // Reset other transform matrices
  D3DXMATRIXA16 identity;
  D3DXMatrixIdentity(&identity);
  myD3DDevice->SetTransform(D3DTS_VIEW, &identity);
  myD3DDevice->SetTransform(D3DTS_WORLD, &identity);
