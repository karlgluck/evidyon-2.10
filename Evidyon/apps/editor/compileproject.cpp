//------------------------------------------------------------------------------------------------
// File:    compileprojectforclient.cpp
//
// Desc:    
//
...
// Desc:  Writes raw data that the client application can use into the destination stream
//------------------------------------------------------------------------------------------------
bool CompileServerResource(const dc::dcList<Projectile>::Element* element, dc::dcStreamOut* stream) {
  return CompileServerResourceStructure<const dc::dcList<Projectile>::Element,
                                  Evidyon::ServerProjectileDescription>(element, stream);
}
bool CompileClientResource(const dc::dcList<Projectile>::Element* element, dc::dcStreamOut* stream) {
  return CompileClientResourceStructure<const dc::dcList<Projectile>::Element,
                                  Evidyon::ClientProjectileDescription>(element, stream);
}


bool CompileServerResource(const dc::dcTable<LifeformAI>::Element* element, dc::dcStreamOut* stream) {
  return CompileServerResourceStructure<const LifeformAI,
...
bool CompileProject(ProjectVolucris* pv, dc::dcStreamOut* clientStream, dc::dcStreamOut* serverStream)
{
    ...
    //// Compile the AI characters
    //if (!CompileServerResourceArray<const dc::dcList<AICharacterType>::Element*>(pv->getAICharacterTypesList()->getMembers(), serverStream))
    //    return false;

    //// Compile the monsters
    //if (!CompileServerResourceArray<const dc::dcList<MonsterClass>::Element*>(pv->getMonsterClassesList()->getMembers(), serverStream))
    //    return false;
    //if (!CompileServerResourceArray<const dc::dcList<MonsterSpawn>::Element*>(pv->getMonsterSpawnList()->getMembers(), serverStream))
    //    return false;
